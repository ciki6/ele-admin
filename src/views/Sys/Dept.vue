  ...
  <div class="page-container">
	...
	<div class="toolbar" style="float:left;padding-top:10px;padding-left:15px;">
		<el-form :inline="true" :model="filters" :size="size">
			...
			<el-form-item>
				<bit-button :label="$t('action.search')" perms="sys:dept:view" type="primary" @click="findTreeData(null)"/>
			</el-form-item>
			<el-form-item>
				<bit-button :label="$t('action.add')" perms="sys:dept:add" type="primary" @click="handleAdd"/>
			</el-form-item>
		</el-form>
	</div>
	<!--表格树内容栏-->
    <el-table :data="tableTreeDdata" stripe size="mini" style="width: 100%;"
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <bit-button :label="$t('action.edit')" perms="sys:dept:edit" @click="handleEdit(scope.row)"/>
          <bit-button :label="$t('action.delete')" perms="sys:dept:delete" type="danger" @click="handleDelete(scope.row)"/>
        </template>
      </el-table-column>
    </el-table>
    <!-- 新增修改界面 -->
   ...
   name: 'Dept',
    ...
    findTreeData: function () {
      this.loading = true
        ...
        this.popupTreeData = this.getParentMenuTree(res.data.content)
        this.loading = false
			})
    },
		// 获取上级机构树
