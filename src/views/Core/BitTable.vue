      ...
      <el-table-column v-for="column in columns" 
       ...
       <el-table-column :label="$t('action.operation')"  width="180" fixed="right" v-if="showOperation">
        <template slot-scope="scope">
          <bit-button :label="$t('action.edit')" :perms="permsEdit" :size="size" @click="handleEdit(scope.$index, scope.row)" />
          <bit-button :label="$t('action.delete')" :perms="permsDelete" :size="size" type="danger" @click="handleDelete(scope.$index, scope.row)" />
        </template>
      </el-table-column>
    </el-table>
    <!--分页栏-->
